...
module.exports = {
    requiredHooks: [ 'generateUserToken', 'generateRefreshToken', 'generateCode', 'userAuthorization', 'validateAuthCode' ],
    ...
    grantCode: function(resourceOwner, redirectUri, responseType, clientId, options){
        ...
        return new Promise(function(resolve, reject){
            ...
            if ( options.allowImplicit && responseType === 'token') {
                ...
                generateCode(resourceOwner, clientId, redirectUri, function(err, code){
                    if (err) return reject(err)
