...
module.exports._setupServer = function(server, options, grants){
    ...
    server.use(_.partialRight(exports.validateToken, options)); 
...
module.exports.validateToken = function (req, res, next, options) {
    ...
    if ( inHeader + inBody + inQuery !== 1 )
        return sendError(new errors.InvalidRequestError('token specified cannot be specified in the header and query string' ));
    
    if ( !token_type || token_type.toLowerCase() !== 'bearer' )
        return sendError(new errors.InvalidRequestError('token_type required' ));
